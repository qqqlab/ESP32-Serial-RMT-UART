...
uint8_t rx_data_buffer[2048+1]; //plus 1 for RingBuffer
Ringbuffer rx_data;
...
IRAM_ATTR void rx_buf_push(uint32_t val) {
  if(rx_data.available() < 4) return;
  rx_data.push( (uint8_t*)&val, 4);
}
...
void rx_print() {
  Serial.printf("len=%d : ",rx_data.used()/4);
  while(rx_data.used() >= 4) {
    qqq_rtm_item32_t sym;
    rx_data.pop((uint8_t*)&sym,4);
...
void tx_send() {
  char *buf = "ABC";  //HHHH LHLLLLLHLH LLHLLLLHLH LHHLLLLHLH HHHHHH == 1L 1H 5L 1H 1L 1H "A" 2L 1H 4L 1H 1L 1H "B" 1L 2H 4L 1H 1L 1H "C"
  Serial1.write((uint8_t*)buf, 3);
...
void setup() {
  ...
  Serial1.begin(10000, SERIAL_8N1, -1, TXPIN); //1bit = 100us
