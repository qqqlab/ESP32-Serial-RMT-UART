...
test case: negative pulse was started before RMT started.
expected result 5 L pulses, first one 500us: 500L 200H 100L 200H 100L 200H 100L 200H 100L 0H
ESP32 = PASS
ESP32S3 = PASS
*/
...
void rx_receive() {
  ...
  //invert pin
  //esp_rom_gpio_connect_in_signal(gpio_num, rmt_periph_signals.groups[0].channels[channel + CHANNEL_OFFSET].rx_sig, true);
  ...
  //non-invert pin (creates start pulse)
  //esp_rom_gpio_connect_in_signal(gpio_num, rmt_periph_signals.groups[0].channels[channel + CHANNEL_OFFSET].rx_sig, false);
}
...
void tx_send() {
  ...
  const uint32_t tdelay = 300*240;
  const uint32_t tlow   = 100*240;
  const uint32_t thigh  = 200*240;
  uint32_t t = esp_cpu_get_cycle_count(); 
  //while(esp_cpu_get_cycle_count() - t < tdelay);
  //t += tdelay;
